...
for index, row in tqdm(qs.iterrows()):
    ...
    for question in questions_cols:
        ...
        q2n = tokenizer.texts_to_sequences(row[question])  # q2n -> numerical vector representation of each question
        tq2n = []
        # Replace questions with equivalent numerical vector/ word-indices
        for i in range(len(q2n)):
            if len(q2n[i]) > 0:
                tq2n.append(q2n[i][0])
        qs.set_value(index, question, tq2n)
    
#function to get back the sample text at specified index
def get_text(index):
    if index >= qs.shape[0]:
        print("Index more than number of samples")
        return
    for i in range(len(qs.iloc[i,:][0])):
        text += tokenizer.index_word[qs.iloc[0,:][0][i][0]] + " "
    return text
...
X_val['right'] = X_val['right'].tolist()
X_val['left'] = X_val['left'].tolist()
# Preparing target labels
y_train = y_train.values
y_val = y_val.values

#Removing single error in the datase
X_train['right'][26] = [x for x in X_train['right'][26] if type(x) == int]
...
embedding_layer = Embedding(len(embeddings), 100, weights=[embeddings], 
...
callback = [EarlyStopping(monitor = 'val_loss',patience=5),
                ...
                CSVLogger(filename='log_autoencoder.csv'),
                ModelCheckpoint('weights.hdf5',
                                save_best_only=True,
                                save_weights_only=True, period = 1),
                TensorBoard(log_dir = 'logs/tensorboard_logs')]

# Start training
for i in range(10):
    malstm_trained = model.fit([X_train['left'], X_train['right']], y_train, batch_size=batch_size, epochs=n_epoch,
                            validation_data=([X_val['left'], X_val['right']], y_val), callbacks=callback)
